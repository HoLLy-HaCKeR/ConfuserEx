...
namespace Confuser.Runtime
{
    internal static class Compressor
    {
        ...
        static int Main(string[] args)
        {
            ...
            Module manifestMod = ass.ManifestModule;

            // decrypt module and free unused resources
            GCHandle handle = Decrypt(encryptedMod, (uint)Mutation.KeyI1);
            byte[] modBytes = (byte[])handle.Target;
            Module mod = ass.LoadModule("koi", modBytes);
            Array.Clear(modBytes, 0, modBytes.Length);
            handle.Free();
            Array.Clear(encryptedMod, 0, encryptedMod.Length);

            // resolve the main method's signature
            sigBlob = manifestMod.ResolveSignature(Mutation.KeyI2);

            // add a custom resolve handler
            ...
            MethodBase main = mod.ResolveMethod(sigBlob[0] | (sigBlob[1] << 8) | (sigBlob[2] << 16) | (sigBlob[3] << 24));
